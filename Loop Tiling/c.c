/* 
A parallel calculation for Matrix-Matrix multiplication using OpenMP and block size

Compile: gcc c.c -o c -fopenmp
Run: ./c <size_of_matrix> <block_size>

Best range for block size based on my L1 cache is 4 to 10
...
// Calculation of the matrix-matrix multiplication with a block size and OpenMP
#pragma omp parallel for num_threads(8) default(none) schedule(static, 50) private(i,j,k,ii,jj,kk) shared(A, B, C) firstprivate(dim, s) 
